...
public class RexProgramRewriter {
	...
	public static class InputBackRewriter extends RexShuttle {
		private int[] usedFields;
		/** old input fields ref index -> new input fields ref index mappings */
		// private Map<Integer, Integer> fieldMap;

		InputBackRewriter(int[] fields) {
		...
		@Override
		public RexNode visitInputRef(RexInputRef inputRef) {
			return new RexLocalRef(refNewIndex(inputRef), inputRef.getType());
		}

		@Override
		public RexNode visitLocalRef(RexLocalRef localRef) {
			return new RexLocalRef(refNewIndex(localRef), localRef.getType());
		}

		private int refNewIndex(RexSlot ref) {
			try{
				return usedFields[ref.getIndex()];
			} catch (IllegalArgumentException e) {
				throw new IllegalArgumentException("input field contains invalid index");
			}
		}
	...
	public static RexNode rewriteRexNode(RexNode condition, int[] usedFields) {
		InputBackRewriter inputBackRewriter= new InputBackRewriter(usedFields);
		RexNode newCondition = condition.accept(inputBackRewriter);
